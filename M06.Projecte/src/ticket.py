...
def guardar_ticket_csv(ticket, rutaTicket):
    
    try:
        # Verificar si el archivo existe para saber si escribir encabezados
        archivo_existe = os.path.isfile(rutaTicket)
        ...
        with open(rutaTicket, 'a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
        ...
        print(f"✅ Ticket guardado en {rutaTicket}")
        return True
