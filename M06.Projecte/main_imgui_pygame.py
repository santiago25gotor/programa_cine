...
#24/10/2025 - Importar funciones actualizadas del sistema de funciones independientes
from sala import (buscar_sala_por_id, 
                  obtener_asientos_funcion, 
                  sala_tiene_asientos_disponibles_funcion,
                  contar_asientos_disponibles_funcion, 
                  asiento_mas_centrado,
                  marcar_asientos_ocupados_funcion, 
                  asientos_a_codigo,
                  guardar_funciones_json, 
                  codigo_asiento)
...
def cargar_fuentes():
    ...
    if font_grande is None:
        print("⚠️ No se encontró fuente con emojis. Usando fuente por defecto.")
...
def cargar_datos():
    ...
    if os.path.isfile(filename):
        with open(filename, "r", encoding="utf-8") as f:
            data = json.load(f)
        #24/10/2025 - Inicializar estructura de funciones si no existe
...
def ventana_peliculas_ui(pos_mouse, eventos):
    ...
    if pelicula_seleccionada:
        ...
        for sala_info in pelicula_seleccionada['salas']:
            ...
            disponible = sala_tiene_asientos_disponibles_funcion(
                dbFilms, pelicula_seleccionada['id'], sala_info['salaId'], sala_info['horario']
            )
    ...
    if sala_completa and sala_seleccionada:
        ...
        asientos_funcion = obtener_asientos_funcion(
        ...
        if asientos_funcion:
            resultado = asiento_mas_centrado(asientos_funcion)
            if resultado:
                f, c = resultado
                                ...
                                if not nombre_usuario.strip():
                                    ...
                                    try:
                                        ...
                                        if marcar_asientos_ocupados_funcion(
                                            ...
                                            generar_ticket(reserva, descuento_aplicado) #24/10/2025 generamos ticket
